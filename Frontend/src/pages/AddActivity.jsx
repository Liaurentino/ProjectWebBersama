      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute top-full left-0 mt-1 w-full bg-white border border-gray-100 rounded-xl shadow-xl z-50 py-1 overflow-hidden">
            ...
            {projects.map((p, i) => (
              <button key={i} type="button" onClick={() => { onChange(p); setOpen(false) }}
            ...
            {!isAdding ? (
              ...
              <div className="px-2 py-2" onClick={e => e.stopPropagation()}>
                <input autoFocus type="text" value={input} onChange={e => setInput(e.target.value)}
                  placeholder="Nama project..." onKeyDown={e => e.key === 'Enter' && handleAdd()}
...
const AddActivity = () => {
  ...
  // FIX: validasi form sebelum boleh start timer
  const isFormReady = title.trim() !== '' && description.trim() !== ''
  ...
  // FIX: start timer hanya kalau form sudah diisi
  const handleStartTimer = () => {
    if (!isFormReady) {
      setError('Isi judul dan deskripsi aktivitas terlebih dahulu sebelum memulai timer.')
      return
    }
    setError(null)
  ...
  if (success) {
    return <SuccessScreen title={savedTitle} onBack={() => navigate('/activity')} onAddMore={handleAddMore} />
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] p-6 sm:p-8">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {/* Mode Toggle */}
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-1 flex gap-1 w-fit">
          ...
          ><PenIcon /> Input Manual</button>
        </div>

        {/* Form Fields — ditaruh SEBELUM timer supaya user isi dulu */}
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" placeholder="Contoh: Belajar UI/UX Design"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <SelectDropdown label="Kategori" value={category} options={CATEGORIES} onChange={setCategory} placeholder="Pilih kategori" />
            <SelectDropdown label="Status" value={status} options={STATUSES} onChange={setStatus} placeholder="Pilih status" />
          </div>

          <ProjectDropdown value={project} onChange={setProject} projects={projects} onAddProject={handleAddProject} />

          <div className="space-y-1.5">
            <label className="text-sm font-medium text-[#434655]">Deskripsi <span className="text-red-400">*</span></label>
            <textarea placeholder="Ceritakan apa yang kamu kerjakan..."
              value={description} onChange={e => { setDescription(e.target.value); setError(null) }}
              rows={4}
        ...
        {mode === 'timer' && (
          <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              {/* FIX: hint kalau form belum diisi */}
              {!isFormReady && (
                <span className="text-xs text-amber-500 font-medium">Isi judul & deskripsi dulu untuk mulai timer</span>
              )}
            </div>
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-2">
                {timerStatus === 'idle' ? (
                  // FIX: disable + style berbeda kalau form belum siap
                  <button type="button" onClick={handleStartTimer}
                    disabled={!isFormReady}
                    className={`w-12 h-12 rounded-xl flex items-center justify-center shadow transition-colors ${
                      isFormReady
        ...
        {mode === 'manual' && (
          <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-6 space-y-4">
            ...
            {manualDurationInvalid && (
              <p className="text-xs text-red-500">Waktu selesai harus lebih besar dari waktu mulai.</p>
            )}
          </div>
        )}
